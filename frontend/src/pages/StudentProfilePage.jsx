...
function StudentProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-background text-text-main">
        ...
        <section className="mx-auto max-w-4xl rounded border border-border-color bg-white p-6">
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              <div className="relative h-16 w-16 overflow-hidden rounded-full border border-border-color bg-background">
                {profile.photoUrl ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-green text-xl font-semibold text-white">
                    {avatarInitial}
                  </div>
                )}
              </div>
            ...
            <div className="text-right text-xs text-text-muted">
              ...
              <div>Edit your university details and contact information.</div>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="mt-6 grid gap-6 md:grid-cols-[1.4fr,1.6fr]">
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-3 space-y-3 text-sm">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold uppercase tracking-[0.18em]">
                      University ID / Reg. No.
                    </label>
                    <input
                      value={profile.identifier}
                      onChange={(e) => handleChange("identifier", e.target.value)}
                      className="mt-1 w-full rounded border border-border-color px-3 py-2 text-sm"
                      placeholder="e.g. 12201234"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold uppercase tracking-[0.18em]">
            ...
            <div className="space-y-4">
                ...
                <div className="mt-3 space-y-3 text-sm">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="mt-1 w-full rounded border border-border-color px-3 py-2 text-sm"
                      placeholder="Link to your photo (hosted image URL)"
                    />
                    <p className="mt-1 text-[11px] text-text-muted">
                      Paste a secure image link from a trusted source. This URL
                      is used only for display inside SCA EMS.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 flex items-center justify-between border-t border-border-color pt-4">
                <div className="text-[11px] text-text-muted">
                  Keep your details aligned with official university records.
                </div>
                <button
                  type="submit"
                  disabled={saving}
      ...
      <SiteFooter
        mode="dark"
        context="Helping School of Computer Applications students keep their event details aligned with faculty expectations."
      />
    </div>
  );
}
